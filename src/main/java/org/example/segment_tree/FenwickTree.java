...
public class FenwickTree {
    ...
    // Построение дерева на основе исходного массива
    public FenwickTree(ArrayList<Integer> array) {
        this.n = array.size();
        this.fenwickTree = new ArrayList<>();
        for (int i = 0; i < n + 1; i++) {
            fenwickTree.add(0L); // Инициализация дерева нулями
        }
        for (int i = 0; i < n; i++) {
            add(i, array.get(i));
        }
    ...
    // Добавление значения value к элементу с индексом idx (0-индексация)
    ...
    // Запрос суммы на отрезке [0, idx]
    public long sum(int idx) {
        idx++; // Переход к 1-индексации
        long result = 0;
        while (idx > 0) {
            result += fenwickTree.get(idx);
            idx -= idx & -idx; // Переход к предыдущему индексу
        }
        return result;
    }
    ...
    public static void main(String[] args) {
        ...
        // Тест производительности дерева Фенвика
        ...
        // Тест производительности прямого суммирования
